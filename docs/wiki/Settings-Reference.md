...
| Pulp | **1d4 luck at the start of each session** | off | Pressing **Start session** in Session Recap sets each player's character to 1d4 luck tokens, as the system's own luck macro does, and posts one card with the rolls. |
...
| Pulp | **Spend luck to make the GM reroll** | off | A player whose character has luck gets a **Luck: force a reroll** button on a GM's roll they can see (not blind, not whispered to others). The roll is redone on the same card, which names who forced it. Costs one luck token. |
