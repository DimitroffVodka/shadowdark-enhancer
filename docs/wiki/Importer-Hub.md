...
- **Mount folder.** Newly imported Mount Actors (single **Import** or the
  **Import everything** / **Import all N in Mounts** batch) are filed into one
  ...
  Already-imported Mounts are not moved. No new API, setting, or pack/selection
  contract.
...
- **Curated Basic Gear icons — D3 activation (44 rows: 37 canonical + 7 aliases, shared `BARE` space).** The Basic Gear map (`scripts/shared/curated-icon-maps/gear-icons.mjs`) is registered through the same discovery index and covers source-agnostic final names. The seven deliberate aliases — `Arrows (20)`, `Caltrops (one bag)`, `Candle (3)`, `Crossbow bolts (20)`, `Iron spikes (10)`, `Rope, morzo silk`, and `Rations (3)` — remain distinct keys that share their canonical icon paths. Matches are stamped `curated`; untouched module-owned art can upgrade, GM-custom art survives, and unmapped gear retains the generic `default` fallback. The composed bare D1+D2+D3 registry is audited directly through `curatedIconRegistry()`: 94 bare rows (37 weapons + 13 armor + 44 Basic Gear), zero duplicate problems or cross-space collisions, with the shared registry now also holding 60 sourced rows across D4–D6. Its structural census always runs in CI; when a Foundry icon inventory is available, the same audit verifies every accepted path (94 bare + 60 sourced). Treasure no longer falls back to generic art: D4 Sea Wolf Plunder, D5 Dead Bandit Loot, and D6 Diabolical Treasure all activate their 20-row source-qualified maps.
...
- **Book-only weapon and armour properties — two outcomes by code (B5).** A property code the core system has no entry for is handled by code: the exact WR **Lance** triple *Charge*/*Devastating*/*Mounted* writes **canonical managed `Property` documents** (and UUIDs) into `Western Reaches / Weapon Properties` via the shared `wr-property-importer` seam shared by **Importer Hub → Manage** (dedicated Items commit and **Commit All**), the guided **Item Builder → Weapons** stage, and the **Paladin** class overlay (`prepareLanceProperties` → `resolveLanceProperties`; see `docs/wiki/Class-and-Spell-Importers.md` for the overlay path). On the **Paladin** overlay the Lance carries those three UUIDs alongside the overlay-seeded `Two-Handed` (live-proved); on direct **Importer Hub** and **Item Builder** pastes the WR weapon table starts `propNames` empty and the materializer appends only the three WR Lance UUIDs, so `Two-Handed` is not synthesized on those roads. Every other WR code with no core property — *Obsidian*/*Sniper* on WR weapons, `Charge` on any non-Lance weapon, and barding's `Mount` (armor `C` is the core property `Occupies One Hand`, not a fallback) — stays **description-only** and is named as *“Properties with no core Shadowdark equivalent: …”* (see `scripts/shared/property-note.mjs`; labels only, no rules text) with the preview flagging the non-materialized remainder in the same way. Reruns across any of the three Lance roads reuse the same folder and Property identities without duplicating either, and a failed Property create is **visible and fail-closed**: `prepareLanceProperties` notifies, leaves the marker for a direct retry, and no owning Item is committed with a partial `system.properties` list. Only an exact `Lance` name materializes the WR triple (e.g. *War lance* remains fallback-labelled and never mints Properties).
...
That was the pre-C1 boundary bug: a `Name. body…` description used to end at the next *anchored* header, so any real record start the anchor list didn't cover — a refused currency row, `Oil flask.` vs the table row `Oil, flask`, or a shared `Rope` variant — was swallowed by the item above it. Page-furniture lines (e.g. `108`) are not record starts; they are excised without splitting a record that continues across the page — the actual following header (known or generic) is what ends the prior body. On the active Item Builder path (`splitDescriptionsByNames` → `record-boundary.mjs`) descriptions end at the next *record start* even when that start is unassigned: a known name plus period (any length on this path, at text start / after a period / after a newline so two records sharing one line still split there) or a line-initial, capitalised, ≤44-char, ≤3-word lead-in that isn't a sentence opener. Trailing prose (`Has a shutter to hide the light.`), wrapped sentences, and lowercase mentions like `iron spikes.` stay with their own record. Assignment aliases and tie-breakers resolve known source variations: a description headed `Oil flask.` assigns to `Oil, flask` via a collision-audited alias (refused if another row also claims the name), and a plain `Rope.` header resolves deterministically to the base `Rope, 60'` row because the alternate row names its material (`Rope, morzo silk`). The `Rope, morzo silk.` header keeps its own item, unresolvable ties remain refused, and repeat matching is idempotent.
