...
- **Legacy consolidation safety net:** The retired legacy pack consolidation continues to run on every activation. If consolidation fails and the retired pack still holds documents, the automatic refresh is deferred to prevent newly generated spells from taking the identity of unmigrated curated originals, and a warning notification is raised once per session directing the GM to manual Build / Refresh. If the retired pack is absent or empty, consolidation errors are logged without blocking the refresh.
