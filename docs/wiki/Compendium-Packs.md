...
### The curated-icon resolver — internal contract (A4, zero production maps)

The same module-owned Items also participate in a shared **curated-icon resolver** introduced in A4. The mechanism is documented here because this pack is where its consumers write; the user-visible behaviour activates in D1–D6, and `CHANGELOG.md` stays unchanged for A4 itself.

The resolver lives in `scripts/shared/curated-icons.mjs` (pure, Foundry-free) with discovery in `scripts/shared/curated-icon-maps/index.mjs`. It **ships no production rows** — only the mechanism and its import-time wiring. Every lookup returns `null` until D1–D6 register their reviewed maps, so no image or A3 provenance classification changes from before.

Why the resolver exists: six issue paths (weapons, armor, basic gear, sea-wolf plunder, dead-bandit loot, diabolical treasure) each want the same thing — a reviewed Foundry-native `icons/**.webp` icon for an item this module imported or generated. The module already has one such resolver per content type (`core-monster-spell-icons.mjs` for Monster Spells); A4 is its generic version rather than six new per-type matchers.
...
Until D1–D6, every call above returns `null` and the behaviour above is fully inert; the only user-visible change under A4 is this documentation.
