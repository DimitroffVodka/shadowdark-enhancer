...
* **Coverage/drift — total construction, audited not thrown.** A duplicate key, blank name, non-`icons/**.webp` path or unusable source never throws; the row is dropped into `problems` and that item keeps fallback art. `auditCuratedIconRegistry()` reports `{total, bare, sourced, perMap, problems, crossSpaceNames}` — `problems` empty is the contract; `crossSpaceNames` (the same prose name appearing in both bare and sourced spaces) is informational, not a failure — a treasure name that also exists as gear resolves qualified-first.
