...
989 tracked files · ~170,300 lines of code/markup across scripts+templates+styles+test.
...
|---|---:|---|
| `shadowdark-enhancer.mjs` | 1119 | **Entry point** (module.json esmodules). Registers hooks, settings, sheets, actor sub-types, the public `game.shadowdarkEnhancer` API, and wires every sub-system. |
...
| `stat-damage/stat-damage-core.mjs` | 113 | Pure stat-damage rules and the effect contract Shadowdark Extras' Effects library follows: a negative ADD on `system.abilities.<key>.value` flagged `statDamage: { ability }`, summed across effects, healed whole or N per ability, and the #181 seam for surviving CON 0. |
| `stat-damage/stat-damage.mjs` | 125 | `statDamage.{apply, heal, of}`: writes replace an ability's effects with one holding the new total (serialized per client), and the active GM marks a character dead when a stat-damage effect takes CON to 0. |
