...
996 tracked files · ~171,500 lines of code/markup across scripts+templates+styles+test.
...
| `settings.mjs` | 697 | All `game.settings.register` calls + migration-safe defaults. |
...
| `setting-groups.mjs` | 178 | Feature groups for Configure Settings — which settings, nested editors, other packages' settings and notes each pop-out shows, in display order, incl. the Modes of Play boxes. Pure data; the docs-contract test imports it. |
