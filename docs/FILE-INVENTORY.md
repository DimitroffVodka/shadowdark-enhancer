...
777 tracked files · ~109,000 lines of code/markup across scripts+templates+styles+test.
...
| `downtime-parser.mjs` | 321 | Parses a pasted downtime page into per-slot outcome text; segment-scoped DC + keyword matching with a rescue pass for column-interleaved PDF copies. Unmatched lines are reported back, never guessed at. |
...
| `downtime-warnings.mjs` | 138 | Shared prose for the downtime parser's warning codes; splits info notes (a two-column paste always emits them) from real problems, so every unlock surface reports a parse identically. |
