...
951 tracked files · ~159,800 lines of code/markup across scripts+templates+styles+test.
...
| `hex-map/hex-tagger-app.mjs` | 1213 | Hex Tagger AppV2: contact sheet over the active hex scene, anchor numbering, tags on the scene flag, dataset hand-off. |
...
| `hex/hex-dataset.mjs` | 185 | Drafts + summary rows + tags → the Shadowdark Extras hexcrawl dataset (numbers only at the boundary). Pure. |
