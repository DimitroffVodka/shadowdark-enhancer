...
| `overland/overland.mjs` | 291 | Overland travel (#229): the overlandState world setting (active-GM writes in one queue, payload-free re-read nudge), Start and End travel (another GM's forwarded to the active GM), the players' relayed Forage with the sender checked from the query context, overland.state() with hexes left, climate, storm, harshness and night derived, rollWeather (GM; dice rolled on the active GM, one chat card, the overlandWeatherRule setting), and the overlandChanged / overlandStart / overlandEnd hooks. |
...
| File | Lines | Description |
|---|---:|---|
| `crawl-bar.mjs` | 716 | GM-only persistent bottom bar above the macro bar (mode toggles, tools, launchers). |
