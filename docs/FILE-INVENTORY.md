...
775 tracked files · ~108,300 lines of code/markup across scripts+templates+styles+test.
...
| `parry/parry.mjs` | 370 | Parry button on an attack card that hit: spends the 1/day use, makes the attack miss, and reverses damage the GM already applied — HP, defeated flag and downed conditions. Player clicks go through the authenticated gm-relay. |
| `taunt/taunt-core.mjs` | 107 | Pure Duelist Taunt rules: round+turn as one ordinal, the "end of your NEXT turn" expiry comparison, advantage/disadvantage cancelling, and what arms the talent (a miss — including a parried hit). |
| `taunt/taunt.mjs` | 215 | Arms Taunt when an enemy misses its holder, sets `mainRoll.advantage` on attacks back at that enemy via `SD-Player-Attack` (with the reason printed on the roll card), and expires it when the holder's next turn ends. |
...
| `attack-card.mjs` | 87 | Reading a Shadowdark attack card — did it land, who was it aimed at, who swung. Shared by Parry and Taunt so the two can never disagree about the target (they once did, silently). |
...
| `char-content/class-unit-importer.mjs` | 1206 | Class unit → real documents in dependency order. |
