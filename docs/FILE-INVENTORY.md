...
992 tracked files · ~170,800 lines of code/markup across scripts+templates+styles+test.
...
| `stat-damage/stat-damage-core.mjs` | 123 | Pure stat-damage rules and the effect contract Shadowdark Extras' Effects library follows: a negative ADD on `system.abilities.<key>.value` flagged `statDamage: { ability }`, summed across effects, healed whole or N per ability, and the #181 seam for surviving CON 0. |
| `stat-damage/stat-damage.mjs` | 135 | `statDamage.{apply, heal, of}`: writes replace an ability's effects with one holding the new total (serialized per client), and the active GM marks a character dead when a stat-damage effect takes CON to 0. |
