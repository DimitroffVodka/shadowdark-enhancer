...
968 tracked files · ~166,700 lines of code/markup across scripts+templates+styles+test.
...
|---|---:|---|
| `shadowdark-enhancer.mjs` | 1096 | **Entry point** (module.json esmodules). Registers hooks, settings, sheets, actor sub-types, the public `game.shadowdarkEnhancer` API, and wires every sub-system. |
...
| `hex-map/hex-region.mjs` | 280 | Which region is a hex in: the book's own word for a keyed hex, the nearest keyed hex's region for any other (84.8% on the Western Reaches, leave-one-out over the book's own keyed rows). Seeds come from the filed crawls; nothing is stored. |
...
| `hex-map/tag-overlay.mjs` | 744 | The tag overlay: every numbered hex drawn on the map in its terrain colour, dots for river/path/coast, an amber ring on unsure automatic cells; hover names a hex, a click edits it through the same scene-flag write. |
...
| File | Lines | Description |
|---|---:|---|
| `importer-hub-app.mjs` | 941 | **The single front door (shell).** ApplicationV2 lifecycle, singleton, instance fields/caches, `_prepareContext`; installs the three method packs below onto the class (split 2026-07-22). |
...
| `importer-hub-manage.mjs` | 1175 | Manage strip: censuses + caches, manage tree, gap/seed/cull, source-PDF grab/extract. |
...
| `tables/hex-parser.mjs` | 452 | Hex-key dumps → per-hex draft journal pages. Pure. |
