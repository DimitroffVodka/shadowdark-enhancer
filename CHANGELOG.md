...
- **Roll Tables: Place button now actually places tokens.** Two bugs compounded:
  1. The click-to-place handler used `ev.data.getLocalPosition(...)` — that's the PIXI v6 API, but Foundry v13 ships PIXI v7+ where federated events have no `.data` field. The handler threw silently inside the PIXI event chain on every click, so nothing ever placed.
  2. Even if the click had worked, compendium actors (e.g. from `shadowdark.bestiary`) can't have tokens placed directly — Foundry needs a world-actor reference for `Token.actorId`.

  Rewrote the flow to match Vagabond's pattern: drop tokens at the **camera viewport center** instead of waiting for a canvas click. The GM scrolls the camera to where they want the tokens, then clicks Place — no PIXI click-event fragility, no timing requirement. Compendium actors are imported as world actors first (or an existing same-named world actor is reused). Tokens land in a 5-column row at 1.5× grid-size spacing. Window stays open so the GM can still Post to chat.
