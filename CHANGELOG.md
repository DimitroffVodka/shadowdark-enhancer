...
- **Sealed content, unlocked with your own books.** The module ships 29 pre-authored, link-verified content units for CS4–CS6 and the *Player's Guide to the Western Reaches* — but **no readable rules text is bundled**. Each unit is **AES-GCM encrypted**, keyed to anchor phrases from the sourcebook; you unlock it by clicking **Unlock** on a missing dashboard entry and **pasting that book's section** (proof you own the book). The verified documents then import exactly as authored, with intra-unit links remapped and **idempotent** de-duplication (re-unlocking never creates duplicates). Covers **spells** (CS4/5/6 lists, WR Necromancer + Priest lists), **monsters** (CS4/5 bestiaries), **gear** (WR Mithral shields, boats, siege, weapons; CS5 gear), **roll tables** (CS6 Carousing/encounters, WR ancestry Name/Trinket tables, spell-mishap tables, Warbands), the **Half-Elf ancestry**, and the 9 WR **classes** (with titles) + backgrounds. Content shared across books (Delver/Wyrdling/Duelist classes; the CS spells reprinted in WR) unlocks from **either** source. Content the Shadowdark **system already ships** (core spells, base bestiary, legacy Bard) is deliberately never sealed.
