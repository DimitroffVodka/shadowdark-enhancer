...
- **Magic Item Forge** — roll or hand-build a magic item (bonuses, benefits, curses, personality, name composition) and create it as a real Foundry item. A **Core Rulebook tables** mode (Phase 1: weapons & armor) reads *your own imported* Core magic-item tables from the managed pack — base recipe (Type/Bonus/Feature), independent Benefit/Curse, and Item Virtue/Flaw/Personality — with per-set readiness, one-click seeded import, roll/pick/re-roll/clear, and a strict fail-closed create. Only a whole-result `+N` becomes a real mechanic (weapon Active Effects / armor AC modifier); other rolls are escaped descriptive text, and the rolled Type is a base-selector hint. Manual forging is unchanged. Treasure/Loot placeholders hand a **stable type hint** into the Forge.
