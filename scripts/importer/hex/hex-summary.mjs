 ...
 *
 * EVERY printed terrain word is read for a feature, including the first. The
 ...
 * books print a keyed hex's feature first whenever there is nothing else to
 ...
 * @returns {{terrain:string, features:string[]}|null}
 */
export function rowTag(row) {
  const terrain = row?.feature;
  if (!terrain) return null;
  const features = (row.terrain ?? []).filter((t) => FEATURE_TAGS.includes(t));
  return { terrain, features };
}

/** Feature tags a keyed row can carry beside its terrain. Matches the tag store's FEATURES. */
export const FEATURE_TAGS = ["river", "path", "coast"];
