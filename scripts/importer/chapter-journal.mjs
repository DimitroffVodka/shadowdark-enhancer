 ...
 * @param {{name?:string, sections?:Array<{name:string, pages:number[]}>|null}} [opts]
 *   `sections` (printed pages) makes each section one page; without it the
 *   text is split at its ALL-CAPS headings.
 * @returns {Array<{key:string, name:string, html:string}>}
 */
export function buildChapterPages(pages, { name = "", sections = null } = {}) {
...
export async function readChapter({ src, pages, name, sections = null }) {
  ...
  return { pages: buildChapterPages(printed, { name, sections: secs }), warnings: result.warnings ?? [] };
}
