...
import { pickShikashiSpellIcon } from "../items/shikashi-icons.mjs";
...
export async function resolveSpellFeatures(draft) {
  ...
  let byName = new Map();
  try {
    const { SpellIndex } = await import("../../monster-creator/spell-index.mjs");
...
export async function resolveDraftArt(draft) {
  try {
    const { _isPlaceholderArt, _findCompendiumActorByName, _getCompendiumArtFor } = await import("../../shared/art-utils.mjs");
