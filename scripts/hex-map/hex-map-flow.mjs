...
import { detectLattice, latticeCentre } from "./lattice.mjs";
...
/**
 * Confirm the detected lattice over a thumbnail; the GM may correct the
 * counts, the lowered parity and the first cell's printed number.
 * @returns {Promise<{cols:number, rows:number, lowered:"odd"|"even", firstNum:string}|null>}
 */
async function confirmLattice(preview, lat, imageW) {
  const pw = preview.width, ph = preview.height, k = pw / imageW;
  const content = `
    <p>Found a hex grid of <strong>${lat.cols} × ${lat.rows}</strong> cells, ${esc(lat.lowered)} columns lowered, hexes ${Math.round(lat.pitchX / 0.75)} × ${Math.round(lat.pitchY)} px. Dots mark cell centres; check the corners.</p>
    <div id="sde-hexmap-preview"></div>
    <div class="form-group"><label>Columns × rows</label><div class="form-fields">
      <input type="number" name="cols" value="${lat.cols}" min="1" max="99" style="width:5rem"> ×
      <input type="number" name="rows" value="${lat.rows}" min="1" max="99" style="width:5rem"></div></div>
    ...
    <div class="form-group"><label>Top-left hex is number</label><input type="text" name="firstNum" value="0000" maxlength="4" style="width:6rem"><p class="hint">The printed number of the top-left cell; every other cell is numbered from it.</p></div>`;
  // DialogV2 sanitises its content and drops <canvas>, so the preview is made here.
  const draw = (root) => {
    const slot = root.querySelector?.("#sde-hexmap-preview"); if (!slot) return;
    const c = document.createElement("canvas");
    c.width = pw; c.height = ph; c.style.cssText = "max-width:100%;height:auto;border:1px solid #888;background:#fff";
    slot.replaceChildren(c);
    const ctx = c.getContext("2d");
    ctx.drawImage(preview, 0, 0);
    ctx.fillStyle = "rgba(220, 30, 30, 0.9)";
    const step = Math.max(1, Math.round(Math.max(lat.cols, lat.rows) / 24));
    for (let col = 0; col < lat.cols; col += step) for (let row = 0; row < lat.rows; row += step) {
      const p = latticeCentre(lat, col, row);
      ctx.beginPath(); ctx.arc(p.u * k, p.v * k, 2.2, 0, Math.PI * 2); ctx.fill();
    }
    ctx.strokeStyle = "rgba(30, 90, 220, 0.9)"; ctx.lineWidth = 1.5;
    const R = lat.pitchX / 1.5 * k, ry = lat.pitchY / 2 * k;
    for (const [col, row] of [[0, 0], [lat.cols - 1, 0], [0, lat.rows - 1], [lat.cols - 1, lat.rows - 1]]) {
      const p = latticeCentre(lat, col, row), x = p.u * k, y = p.v * k;
      ctx.beginPath();
      for (const [dx, dy] of [[R, 0], [R / 2, ry], [-R / 2, ry], [-R, 0], [-R / 2, -ry], [R / 2, -ry]]) ctx.lineTo(x + dx, y + dy);
      ctx.closePath(); ctx.stroke();
    }
  };
  const answer = await foundry.applications.api.DialogV2.wait({
    window: { title: "Hex grid found" },
    position: { width: Math.min(760, pw + 60) },
    content,
 ...
 */
export function alignedSceneData({ name, src, imageW, imageH, lat, firstNum = "0000", cols, rows, levels = sceneHasLevels() }) {
  ...
  state.origin = { i: 0, j: 0, q: cube.q, r: cube.r, num: firstNum, shifted: lat.lowered, bounds: { cols, rows } };
...
export async function startHexMapFlow() {
  ...
  try {
    ...
    const s = w / imageW;
    const lat = { x0: det.x0 / s, y0: det.y0 / s, pitchX: det.pitchX / s, pitchY: det.pitchY / s, cols: det.cols, rows: det.rows, lowered: det.lowered };
    const preview = await createImageBitmap(working, { resizeWidth: 640, resizeHeight: Math.round(640 * imageH / imageW), resizeQuality: "medium" });
    const answer = await confirmLattice(preview, lat, imageW);
    preview.close?.();
    if (!answer) return null;
    const src = await uploadMap(file);
    const data = alignedSceneData({ name, src, imageW, imageH, lat: { ...lat, lowered: answer.lowered }, firstNum: answer.firstNum, cols: answer.cols, rows: answer.rows });
    const scene = await Scene.create(data);
    ui.notifications?.info(`Scene "${scene.name}" created: ${answer.cols} × ${answer.rows} hexes on a ${data.grid.size} px grid. Opening the tagger.`);
    await scene.view();
    (await import("./hex-tagger-app.mjs")).HexTaggerApp.open();
    return scene;
  } catch (err) {
